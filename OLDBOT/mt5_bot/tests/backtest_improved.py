...
def add_indicators(df, ema_period=50, atr_period=14, adx_period=14, symbol: Optional[str]=None, use_cache: bool=True):
    ...
    df['EMA_H1'] = cs.rolling(12).mean().ffill().fillna(0).values  # 12x M5 = 1hr
    df['EMA_8'] = cs.ewm(span=8, adjust=False).mean().ffill().fillna(0).values
    df['EMA_21'] = cs.ewm(span=21, adjust=False).mean().ffill().fillna(0).values
    df['EMA_34'] = cs.ewm(span=34, adjust=False).mean().ffill().fillna(0).values
    df['EMA_50'] = cs.ewm(span=50, adjust=False).mean().ffill().fillna(0).values
