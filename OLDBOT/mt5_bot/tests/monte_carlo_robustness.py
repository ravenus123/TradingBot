...
STRATEGIES_DIR = BOT_DIR / 'strategies'
import sys
...
STRATEGY_MAP = {
    ...
    'candle_reversal': lambda df_h1, df_m5, symbol, sym_info, params: __import__('candle_reversal_strategy').generate_candle_reversal_signal(df_h1, df_m5, symbol, sym_info, params),
    'sigma_mean_reversion': lambda df_h1, df_m5, symbol, sym_info, params: __import__('sigma_mean_reversion_strategy').generate_sigma_mean_reversion_signal(df_h1, df_m5, symbol, sym_info, params),
    'liquidity_sweep': lambda df_h1, df_m5, symbol, sym_info, params: __import__('liquidity_sweep_strategy').generate_liquidity_sweep_signal(df_h1, df_m5, symbol, sym_info, params),
    'session': lambda df_h1, df_m5, symbol, sym_info, params: __import__('session_strategy').generate_session_signal(df_h1, df_m5, symbol, sym_info, params),
    'momentum_premium': lambda df_h1, df_m5, symbol, sym_info, params: __import__('momentum_premium_strategy').generate_momentum_premium_signal(df_h1, df_m5, symbol, sym_info, params),
    'enhanced_mean_reversion': lambda df_h1, df_m5, symbol, sym_info, params: __import__('enhanced_mean_reversion').generate_enhanced_mean_reversion_signal(df_h1, df_m5, symbol, sym_info, params),
    'simple_momentum': lambda df_h1, df_m5, symbol, sym_info, params: __import__('simple_momentum').generate_simple_momentum_signal(df_h1, df_m5, symbol, sym_info, params),
}
...
def load_candidates(limit: int | None = None, config_file: str = 'production_strategy_lock.json') -> List[Dict]:
    ...
    # Test configs are in tests folder, production config is in liverun/config
    if config_file == 'production_strategy_lock.json':
...
def run_monte_carlo_test(candidates: List[Dict], outdir: Path, bars: int = 50000, 
    ...
    for sim in range(num_simulations):
        ...
        max_start_offset = max(0, bars - (period_days * 96))  # 96 M15 bars per day
        if max_start_offset <= 0:
            max_start_offset = 0
        start_offset = random.randint(0, max_start_offset) if max_start_offset > 0 else 0
...
def run_walk_forward_analysis(candidates: List[Dict], outdir: Path, bars: int = 50000,
                              risk_pct: float = 0.2, config_file: str = 'production_strategy_lock.json') -> Dict:
    """Run walk-forward analysis with rolling in-sample/out-of-sample windows."""
    print("Running Walk-Forward Analysis")
    print(f"Candidates: {len(candidates)}")
    
    # Walk-forward parameters
    is_days = 60  # In-sample period (days)
    oos_days = 30  # Out-of-sample period (days)
    step_days = 30  # Step size for rolling window (days)
    ...
    total_bars_needed = (is_days + oos_days) * bars_per_day
    if bars < total_bars_needed:
        print(f"Warning: Not enough data for walk-forward. Need {total_bars_needed} bars, got {bars}")
        bars = total_bars_needed
    
    # Calculate number of walk-forward windows
    max_windows = (bars - is_days * bars_per_day) // (step_days * bars_per_day)
    max_windows = min(max_windows, 10)  # Limit to 10 windows
    
    for window in range(max_windows):
        print(f"\nWalk-Forward Window {window + 1}/{max_windows}")
        
        # Calculate window boundaries
        is_start = window * step_days * bars_per_day
        is_end = is_start + is_days * bars_per_day
        oos_start = is_end
        oos_end = oos_start + oos_days * bars_per_day
        
        if oos_end > bars:
            print("  Not enough data for out-of-sample period, stopping")
            break
        ...
        for symbol in set(c["symbol"] for c in candidates):
            df = fetch_data(symbol, bars=bars)
            if df is not None and len(df) >= oos_end:
                # Slice to out-of-sample period
                df_oos = df.iloc[oos_start:oos_end].copy()
                instrument_data[symbol] = df_oos
        
        if not instrument_data:
            continue
        
        # Run all candidates on OOS period
        portfolio_equity = pd.Series([10000.0])
        all_trades = []
        ...
        metrics['is_period'] = f"Bars {is_start}-{is_end}"
        metrics['oos_period'] = f"Bars {oos_start}-{oos_end}"
...
def run_monte_carlo():
    ...
    parser.add_argument("--walk-forward", action="store_true", help="Run walk-forward analysis instead of Monte Carlo")
    ...
    if args.walk_forward:
        ...
        result = run_walk_forward_analysis(
            candidates, outdir, args.bars, args.risk_pct, args.config
        )
        ...
        print(f"IS Period: {result['is_days']} days")
        print(f"OOS Period: {result['oos_days']} days")
        ...
        summary = {
            'config': {
                'is_days': result['is_days'],
                'oos_days': result['oos_days'],
        ...
        (outdir / 'walk_forward_summary.json').write_text(json.dumps(summary, indent=2), encoding='utf-8')
        
        # Plot return distribution across windows
        plt.figure(figsize=(12, 6))
        plt.bar(range(1, len(returns) + 1), returns, alpha=0.7, edgecolor='black')
        plt.axhline(np.mean(returns), color='red', linestyle='--', label=f'Mean: {np.mean(returns):.2f}%')
        plt.axhline(0, color='green', linestyle='-', label='Break Even')
        plt.xlabel('Walk-Forward Window')
        plt.ylabel('Return %')
        plt.title(f'Walk-Forward Returns ({len(returns)} Windows)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(outdir / 'walk_forward_returns.png', dpi=150)
        plt.close()
