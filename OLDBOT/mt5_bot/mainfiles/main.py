...
sys.path.insert(0, str(Path(__file__).parent.parent / 'not-used'))
...
from hedgefund_data_collector import init_data_collector, get_data_collector

# Database initialization
from db import init_trading_db, log_event

# Telegram bot
import telegram_bot
TELEGRAM_BOT_TOKEN = telegram_bot.TELEGRAM_BOT_TOKEN
TELEGRAM_CHAT_ID = telegram_bot.TELEGRAM_CHAT_ID
send_telegram_message = telegram_bot.send_telegram_message
...
# Config
PRODUCTION_STRATEGY_LOCK_FILE = Path(__file__).parent.parent / 'liverun' / 'config' / 'production_strategy_lock.json'
...
def get_symbol_info(symbol: str) -> dict | None:
    ...
    return {
        ...
        'trade_stops_level': getattr(info, 'trade_stops_level', 0),  # Minimum distance for SL/TP
...
def place_order(signal: dict, sym_info: dict, lot_size: float) -> dict:
    ...
    try:
        ...
        # Get minimum distance from broker's trade_stops_level
        # If trade_stops_level is 0, use 10 ticks as fallback
        trade_stops_level = sym_info.get('trade_stops_level', 0)
        if trade_stops_level > 0:
            min_distance = trade_stops_level
        else:
            min_distance = sym_info.get('trade_tick_size', 0.00001) * 10  # 10 ticks fallback
        
        # Adjust SL if too close
        adjusted_sl = signal['stop']
        if stop_dist < min_distance:
            # Move SL further away to meet minimum distance
            if signal['direction'] == 'BUY':
                adjusted_sl = price - min_distance
            else:
                adjusted_sl = price + min_distance
            print(f"[ADJUST] {signal.get('symbol')}: SL adjusted from {signal['stop']:.5f} to {adjusted_sl:.5f} (min distance: {min_distance:.5f})")
        
        # Adjust TP if too close
        adjusted_tp = signal['tp']
        if tp_dist < min_distance:
            # Move TP further away to meet minimum distance
            if signal['direction'] == 'BUY':
                adjusted_tp = price + min_distance
            else:
                adjusted_tp = price - min_distance
            print(f"[ADJUST] {signal.get('symbol')}: TP adjusted from {signal['tp']:.5f} to {adjusted_tp:.5f} (min distance: {min_distance:.5f})")
        
        # Update signal with adjusted SL/TP
        signal['stop'] = adjusted_sl
